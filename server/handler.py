...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if "/get-messages" in self.path: # Here make it so that each client has its own page with its username/session_id
            if self.valid_auth_cookie():
                # Encapsulate this in a different method, perhaps a different class even ?

                self.server_engine.request_messages(
                    self.__class__.active_sessions[cookies.SimpleCookie(self.headers['Cookie'])["session-id"].value])
